...
| Footer  | Social icons hidden (`SHOW_SOCIAL = false`). **TODO: social URLs** | No profile URLs yet — client decision |
| Links   | No `href="#"` anywhere: on-page topics anchor to sections (`/#features`, `/#deliver-projects`, `/#strategic-planning`, `/#testimonials`, `/#integrations`); everything else → `/coming-soon` (noindex). Map lives in `content/routes.ts` + `content/navigation.ts` | Client decision — replace coming-soon targets as real pages ship |
...
| Features / Logo | Six icon tiles + logo mark shipped as 2× WebP (~2.4KB) instead of the Figma SVG export (~43KB each) | Performance; SVGs carried a 73-path near-invisible dot grid. Mean pixel diff vs SVG 2.5/255 (edge anti-aliasing) |
| Status pages | `/coming-soon` and the 404 page have no Figma frames — built only from tokens + existing primitives (Badge, Button, display type) | Needed for real links (no `href="#"`) and a branded 404 |
| SEO assets | OG/Twitter image (1200×630), favicon, apple-touch and PWA icons have no Figma source — derived from the logo glyph + brand gradient | Required metadata; only visible-on-page claims used |
| Testimonials | <1280 swipe track is keyboard-focusable (inset focus ring) and named for screen readers | axe `scrollable-region-focusable`; no visual change until focused |
